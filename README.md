...
Although now solved (2025), I'll leave my description of these three related mysteries in here as a record of the learning process. The essence of my misunderstanding was that I didn't understand the fundamentals of CSS animation. I underestimated the degrees of freedom. In particular, I didn't realise that there are two ways to divide up the total duration. One is keyframes, marked with %. Keyframes specify what state the element will be in, starting from the given percentage of the total duration and ending at the next keyframe. By default, values transition smoothly between the values specified at keyframes. When only a 0% keyframe is given, there is an implicit 100% keyframe that has the original, pre-animation style.
...
One final curiosity: When I initially did this, I left the total duration at 1s, forgetting that it was in fact twice as fast in the original. This had the side effect that, for the first few seconds of holding down the space bar (fire) and hitting aliens, all aliens stopped flapping for most of each second and just flapped on what looks like half of the second. One suggestion was that this is jank due to them being better synchronized, but I'm not convinced; the pattern was so regular and dev tools showed no especial strain there. It only happened when I held fire, not when I fired shots one by one. It didn't happen when I held fire without hitting aliens. I suspect it's something to do with the mechanism whereby the aliens speed up the more of them are hit, that maybe hadn't been brought into line with the new way of doing things, but commenting out the `if (aliensDanceFaster) {` block didn't resolve it.
