...
const gameOverView = (state) => {
  ...
  <div id="end-game-prompt">
    ${
      state.player.score > Math.min(...state.scores.map((el) => el.score))
        ? `<form id="score-form">
    <div class="gameOver-stats">
        <div class="stat-group">
            <label for="score1" class="stat-label">SCORE:</label>
            <p class="stat-input" id="score1" name="score">${state.player.score}</p>
        </div>
        <div class="stat-group">
            <label for="time" class="stat-label">TIME:</label>
            <p class="stat-input" id="time" name="time">${state.timer.timer}</p>
        </div>
    </div>
  ...
  <button class="restartButton" id="restartButton">start new Game</button>
  `;
...
async function updatesGameOver() {
  ...
  audioContext.suspend()
  gameOverView(state);
  if (state.player.score < Math.min(...state.scores.map((el) => el.score))) {
    setTimeout(() => {
      displayScoreboard(state.scores, message);
      showScoreBoard();
    }, 1024);
  }
...
const controlScore = async (obj) => {
  try {
    ...
    console.log(obj);
    state.player.name = obj.playerName;
    displayScoreboard(state.scores, message);
    showScoreBoard();
  } catch (err) {
    console.log(err);
...
function addScore({name, score, minutes, seconds}) {
  ...
  state.scores.push({name, score, time});
...
const deleteMinimumScore = () => {
  const minValue = Math.min(...state.scores.map((el) => el.scores));
  const minIndex = state.scores.indexOf(minValue);
  state.scores.splice(minIndex, 1);
};

const updateScoresOnAdd = ({playerName, score, second, minute}) => {
  const time = formatTime(minute, second);
  state.scores.push({name: playerName, score: +score, time});
  state.scores.sort((a, b) => b.score - a.score);
};

const message = () => {
  // const highestScore = state.scores[0].score;
  const position = state.scores.findIndex(
    (el) => el.score === state.player.score && state.player.name === el.name
  );
  ...
  const percentage = (position / state.scores.length) * 100;
  let suffix = "";
  ...
  return `Congrats, ${state.player.name}! You are in the top ${Math.round(
    percentage
