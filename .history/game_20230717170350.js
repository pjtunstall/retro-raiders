...
const controlScore = async (obj) => {
  try {
    ...
    const overlay = (document.getElementById("overlay").innerHTML = "");

    playerName = obj.playerName;

    gameContainer.style.visibility = "none";
    displayScoreboard(scores, message);
    showScoreBoard();
  } catch (err) {
    console.log(err);
...
const message = () => {
  ...
  const position = scores.findIndex(
    (el) => el.score === player.score && player.name === el.name
  );
  ...
  return `Congrats, ${player.name}! You are in the top ${Math.round(
    percentage
