...
function reset(restart) {
  ...
  ufo.gettingPlayer = false;
  ufo.toggoleBeam = false;
  ufo.takenPlayer = false;
...
function update(frameDuration) {
  console.log(ufoToggleBeam, ufoGetPlayer);
  // Update fades.
  if (fadeOption) {
    let brightest = 1;
  ...
  if (
    (ufoGetPlayer && ufoActive && ufoLeft < -ufoWidth - 8) ||
    ufoLeft > containerWidth + 8
  ) {
    showElementsOnBeam();
    setTimeout(() => {
      ufoToggleBeam = false;
